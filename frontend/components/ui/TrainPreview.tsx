...
import { COACH, coachGeometry, coachMaterial } from "../three/coachGeometry";
...
export function TrainPreview({ lineColor }: { lineColor: string }) {
  const lead = useMemo(() => coachGeometry("cab", lineColor), [lineColor]);
  const trailer = useMemo(() => coachGeometry("trailer", lineColor), [lineColor]);
